...
def google_config_init(
    client_secrets_file: Union[str, Path], encoding: str = "utf8"
) -> GoogleAuth:
    ...
    if not os.path.isfile(client_secrets_file):
        raise ValueError(
            f"Credentials not found at {client_secrets_file}. Please provide a valid"
            " path!"
        )
...
def _check_google_config() -> None:
    ...
    for f in [CLIENT_SECRETS_PATH, CREDENTIALS_PATH, SETTINGS_PATH]:
        if not os.path.isfile(f):
            raise FileNotFoundError(
                f"{f} file was not found. Please check you have run"
                " `google_config_init`!"
            )
