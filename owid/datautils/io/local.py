...
def _load_json_data_and_duplicated_keys(ordered_pairs: List[Tuple[Hashable, Any]]) -> Tuple[Dict[Any, Any], List[Any]]:
    clean_dict = {}
    duplicated_keys = []
...
def load_json(json_file: Union[str, Path], warn_on_duplicated_keys: bool = True):
    ...
    with open(json_file, "r") as _json_file:
        if warn_on_duplicated_keys:
            data, duplicated_keys = json.loads(_json_file.read(), object_pairs_hook=_load_json_data_and_duplicated_keys)
            if len(duplicated_keys) > 0:
                warn_on_list_of_entities(duplicated_keys, f"Duplicated entities found in {json_file}", show_list=True)
        else:
